...
export const patientConditionApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getConditions: builder.query<PatientCondition[], PatientConditionSearchFilters>({
      query: (filters) => ({
        url: '',
        params: filters,
      }),
    ...
    getConditionsByPatientId: builder.query<PatientCondition[], string>({
      ...
      providesTags: (result, error, patientId) => [
        ...
        ...(result || []).map(({ id }) => ({ type: 'PatientCondition' as const, id })),
      ],
    }),

    createCondition: builder.mutation<PatientCondition, CreatePatientConditionDto>({
      query: (data) => ({
        url: '',
        method: 'POST',
        body: data,
    ...
    updateCondition: builder.mutation<PatientCondition, { id: string; data: UpdatePatientConditionDto }>({
      query: ({ id, data }) => ({
        url: `/${id}`,
        method: 'PATCH',
        body: data,
    ...
    // Bulk operations
    createConditionsForPatient: builder.mutation<PatientCondition[], { patientId: string; conditions: Omit<CreatePatientConditionDto, 'patientId'>[] }>({
    ...
    updatePatientConditions: builder.mutation<PatientCondition[], { patientId: string; conditions: CreatePatientConditionDto[] }>({
      query: ({ patientId, conditions }) => ({
