...
class ViewportGridService {
  ...
  public addPresentationIdProvider(
    id: string,
    provider: PresentationIdProvider
  ): void {
   ...
   */
  public getPresentationIdProvider(id: string): PresentationIdProvider | undefined {
  ...
  private _getPresentationId(id : string, { viewport, viewports }: { viewport: any; viewports: Map<string, any>; }) {
  ...
  public getPresentationIds({ viewport, viewports }: { viewport: any; viewports: Map<string, any>; }) {
    // Use the keys of the Map to get all registered provider IDs
    const registeredPresentationProviders = Array.from(
      this.presentationIdProviders.keys()
    );
  ...
  public setServiceImplementation({
    ...
    if (getViewportStateImplementation) {
      this.serviceImplementation._getViewportState =
        getViewportStateImplementation;
    }
    ...
    if (setViewportIsReadyImplementation) {
      this.serviceImplementation._setViewportIsReady =
        setViewportIsReadyImplementation;
    }
  }

  public publishViewportsReady() {
    this._broadcastEvent(ViewportGridService.EVENTS.VIEWPORTS_READY, {});
  }

  public publishViewportOnDropHandled(eventData: any) {
    this._broadcastEvent(ViewportGridService.EVENTS.VIEWPORT_ONDROP_HANDLED, { eventData });
  }
  ...
  public setActiveViewportId(id: string) {
    ...
    setTimeout(() => {
      this._broadcastEvent(ViewportGridService.EVENTS.ACTIVE_VIEWPORT_ID_CHANGED, {
        viewportId: id,
      });
    }, 0);
  }
  ...
  public setViewportIsReady(viewportId: any, callback: any) {
  ...
  public setViewportGridSizeChanged() {
    const state = this.getState();
    this._broadcastEvent(ViewportGridService.EVENTS.GRID_SIZE_CHANGED, {
      state,
    });
  }

  public setDisplaySetsForViewport(props: any) {
    // Just update a single viewport, but use the multi-viewport update for it.
    this.setDisplaySetsForViewports([props]);
  }

  public async setDisplaySetsForViewports(viewportsToUpdate: any) {
    await this.serviceImplementation._setDisplaySetsForViewports(
      viewportsToUpdate
    );
    const state = this.getState();
    const updatedViewports: any[] = [];

    const removedViewportIds: any[] = [];
    ...
    for (const viewport of viewportsToUpdate) {
      ...
      if (updatedViewport) {
        ...
        const updatedDisplaySetUIDs =
          updatedViewport.displaySetInstanceUIDs || [];
  ...
  public async setLayout({
    ...
    setTimeout(() => {
      ...
      const removedViewportIds = [...prevViewportIds].filter(
        (id) => !currentViewportIds.has(id)
      );

      this._broadcastEvent(ViewportGridService.EVENTS.LAYOUT_CHANGED, {
        numCols,
        numRows,
      });
  ...
  public getLayoutOptionsFromState(
    ...
    return (Array.from(state.viewports.entries()) as [any, any][]).map(([_, viewport]) => {
      return {
        x: viewport.x,
        y: viewport.y,
