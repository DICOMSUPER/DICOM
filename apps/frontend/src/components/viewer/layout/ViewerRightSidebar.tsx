...
import React, { useState, useEffect } from 'react';
import { Filter, FolderOpen, Database, Grid3X3, List, FileText, Loader2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';
import { DicomSeries } from '@/interfaces/image-dicom/dicom-series.interface';
  ...
  useEffect(() => {
    ...
    const loadAnnotationsAndSegmentations = async () => {
      ...
      if (activeSeries?.id === selectedSeries) {
        ...
        const currentSnapshots = segmentationLayers.map(layer => layer[layer.length - 1])
  ...
  return (
    <TooltipProvider>
      <div className="bg-linear-to-b from-slate-900 via-slate-900 to-slate-950 border-l-2 border-teal-900/30 flex flex-col h-full shadow-2xl">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    size="sm"
                    onClick={() => setViewMode('grid')}
                    className={`h-6 w-6 p-0 transition-all ${viewMode === 'grid' ? 'text-teal-300 bg-teal-900/40' : 'text-slate-400 hover:text-white'}`}
                  >
                    <Grid3X3 className="h-3.5 w-3.5" />
                  </Button>
                </TooltipTrigger>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    size="sm"
                    onClick={() => setViewMode('list')}
                    className={`h-6 w-6 p-0 transition-all ${viewMode === 'list' ? 'text-teal-300 bg-teal-900/40' : 'text-slate-400 hover:text-white'}`}
                  >
                    <List className="h-3.5 w-3.5" />
                  </Button>
                </TooltipTrigger>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className={`h-6 w-6 p-0 transition-all ${viewMode === 'annotations' ? 'text-teal-300 bg-teal-900/40' : 'text-slate-400 hover:text-white'}`}
                  >
                    <FileText className="h-3.5 w-3.5" />
                  </Button>
                </TooltipTrigger>
                <TooltipContent className="bg-slate-700 border-slate-600">Annotations & Segmentations</TooltipContent>
              </Tooltip>
            </div>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  size="sm"
                  onClick={() => setShowFilter(!showFilter)}
                  className={`h-8 w-8 p-0 transition-all rounded-lg ${showFilter ? 'text-teal-300 bg-teal-900/40 border border-teal-700/50' : 'text-white hover:text-teal-300 hover:bg-slate-800 border border-transparent'}`}
                >
                  <Filter className="h-4 w-4" />
                </Button>
              </TooltipTrigger>
        ...
        <div className="flex-1 overflow-y-auto">
          {viewMode === 'annotations' ? (
            <AnnotationsSegmentationsList annotations={annotations} segmentations={segmentations} />
          ) : (
            <div className="p-2 space-y-1">
              {/* Replace with actual series rendering logic */}
              <SeriesCard seriesId={selectedSeries} onClick={handleSeriesClick} />
            </div>
          )}
        </div>
      </div>
