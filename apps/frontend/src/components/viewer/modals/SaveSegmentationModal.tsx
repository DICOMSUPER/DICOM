...
import { AlertTriangle, Save, Layers, CheckCircle2 } from "lucide-react";
import { Switch } from "@/components/ui/switch";

interface SaveSegmentationModalProps {
    open: boolean;
...
export function SaveSegmentationModal({
    open,
    onOpenChange,
    layer,
    onConfirm,
}: SaveSegmentationModalProps) {
            ...
            <DialogContent className="max-w-md bg-slate-900 border-slate-700 text-white">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-slate-400">
                        Choose how to save this segmentation layer
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4 py-4">
                    ...
                    {saveAsFinal && (
                        <Alert className="bg-amber-900/20 border-amber-700">
                            ...
                            <AlertDescription className="text-xs text-amber-200">
                                <strong>Note:</strong> Once marked as Final and reviewed by a physician, this layer will become read-only and cannot be modified.
                            </AlertDescription>
                        </Alert>
                    )}
                    ...
                    <div className="flex items-start space-x-2 p-3 rounded-lg bg-slate-800/50 border border-slate-700">
                        ...
                        <div className="space-y-1">
                            ...
                            >
                                I confirm saving this segmentation layer
                            </Label>
                            <p className="text-xs text-slate-400">
                                {saveAsFinal
                                    ? "I understand this will be submitted as Final"
                                    : "This layer will be saved as a Draft"}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="text-xs text-slate-400 p-2 rounded bg-slate-800/30 space-y-1">
                        <div><strong>Name:</strong> {layer.metadata.name || "Untitled Layer"}</div>
                        <div><strong>Snapshots:</strong> {layer.snapshots?.length || 0}</div>
                    </div>
                </div>
