...
const MedicalRecordMain = ({
  ...
  const handleCreateDiagnosis = async () => {
    if (!selectedStudyId || !encounterId)
      return alert("Thiếu study hoặc encounter ID!");
  ...
  if (!selectedStudyId)
    return (
      <div className="flex-1 flex items-center justify-center text-gray-500">
        Chưa có Study — hãy tạo mới chẩn đoán.
      </div>
    );
            ...
            {[
              ...
              <TabsTrigger
                onClick={() => {
                  if (tab.value === "view")
                    toast.info(
                      "Please wait for viewer to load. This might take a while "
                    );
                  router.push(`/viewer?study=${selectedStudyId}`);
                }}
                key={tab.value}
                value={tab.value}
