...
const getShownTours = () => {
  const stored = localStorage.getItem('shownTours');
...
const defaultShowHandler = (Shepherd: ShepherdBase) => {
  ...
  if (currentStep && Shepherd?.activeTour?.steps) {
    const progress = document.createElement('span');
    progress.className = 'shepherd-progress text-lg text-foreground';
...
const customMiddleware = {
  ...
  async fn(state : any) {
    const overflow = await detectOverflow(state, {
      boundary: document.querySelector('body') || document.documentElement,
      padding: 24,
    });
