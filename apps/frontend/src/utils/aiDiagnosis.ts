  ...
  try {
    ...
    if (foundToolGroup) {
      if (!foundToolGroup.hasTool(PlanarFreehandROITool.toolName)) {
        console.log("Adding PlanarFreehandROI tool to group");
        foundToolGroup.addTool(PlanarFreehandROITool.toolName);
      }
      // Đặt Passive để hiển thị annotation mà không cần active tool
      const toolState = foundToolGroup.getToolOptions(
        PlanarFreehandROITool.toolName
      );
      if (!toolState || toolState.mode !== "Active") {
        // Giữ Active nếu đang vẽ dở, nếu không thì Passive
      ...
      if (prediction.points && prediction.points.length > 0) {
        ...
        const polygonAnnotation: Annotation = {
          ...
          metadata: {
            ...
            // enabledElement: enabledElement || undefined,
    ...
    setTimeout(() => {
      console.log("Delayed render 1");
      viewport.render();
    }, 50);

    setTimeout(() => {
      console.log("Delayed render 2");
      viewport.render();
    }, 150);
