...
import { PatientConditionRepository, PatientCondition } from '@backend/shared-domain';
import {
...
export class PatientConditionService {
  constructor(
    @Inject() private readonly patientConditionRepository: PatientConditionRepository
  ) {}

  private checkPatientCondition = async (id: string): Promise<PatientCondition> => {
  ...
  create = async (
    ...
    return await this.patientConditionRepository.create(createPatientConditionDto);
  };
  ...
  update = async (
    ...
    return await this.patientConditionRepository.update(id, updatePatientConditionDto);
  };
