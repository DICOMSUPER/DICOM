...
export class EmployeeScheduleController {
  ...
  async update(
    @Payload() data: { id: string; updateDto: UpdateEmployeeScheduleDto }
  ) {
    try {
  ...
  async findByCurrentUser(
    ...
    try {
      return await this.employeeScheduleService.findByCurrentUser(
        data.userId,
        data.limit,
        data.start_date,
        data.end_date
  ...
  async findWithFilters(
    @Payload() data: { filters: EmployeeScheduleSearchFilters }
  ) {
    try {
  ...
  async createBulk(
    @Payload() data: { schedules: CreateEmployeeScheduleDto[] }
  ) {
    try {
  ...
  async copyWeek(
    ...
    try {
      return await this.employeeScheduleService.copyWeek(
  ...
  async checkConflict(
    ...
    try {
      return await this.employeeScheduleService.checkConflict(
        data.employeeId,
        data.date,
        data.startTime,
        data.endTime,
  ...
  async validateAgainstWorkingHours(
    @Payload() data: { schedules: EmployeeSchedule[] }
  ) {
    try {
      return await this.employeeScheduleService.validateAgainstWorkingHours(
        data.schedules
      );
    } catch (error) {
      throw handleErrorFromMicroservices(
  ...
  @MessagePattern('UserService.EmployeeSchedule.GetOverlappingSchedule')
  async getOverlappingSchedules(@Payload() data: { id: string; role: Roles }) {
    return await this.employeeScheduleService.getOverlappingSchedules(
      data.id,
      data.role
    );
  }
