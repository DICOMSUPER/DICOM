...
import { BodyPart, CreateServiceDto, Services, UpdateBodyPartDto, UpdateServiceDto } from '@backend/shared-domain';
...
export class ServicesController {
  ...
  async create(
    ...
    this.logger.log(
      `Using pattern: UserService.Services.Create`
    );
    try {
  ...
  @MessagePattern(
    `UserService.Services.FindAll`
  )
  async findAll(): Promise<Services[]> {
    this.logger.log(
      `Using pattern: UserService.Services.FindAll`
    );
    try {
  ...
  @MessagePattern(
    `UserService.Services.FindOne`
  )
  async findOne(
    @Payload() data: { id: string }
  ): Promise<Services | null> {
    this.logger.log(
      `Using pattern: UserService.Services.FindOne`
    );
    try {
      const { id } = data;
  ...
  async update(
    ...
    this.logger.log(
      `Using pattern: UserService.Services.Update`
    );
    try {
  ...
  async remove(@Payload() data: { id: string }): Promise<boolean> {
    this.logger.log(
      `Using pattern: UserService.Services.Delete`
    );
    try {
      const { id } = data;
